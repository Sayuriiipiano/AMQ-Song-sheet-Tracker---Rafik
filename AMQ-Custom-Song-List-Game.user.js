// ==UserScript==
// @name         AMQ Custom Song List Game Bulk Search - Sayurii
...
// @downloadURL  https://raw.githubusercontent.com/Sayuriiipiano/AMQ-Song-sheet-Tracker---Rafik/main/AMQ-Custom-Song-List-Game.user.js
// @updateURL    https://raw.githubusercontent.com/Sayuriiipiano/AMQ-Song-sheet-Tracker---Rafik/main/AMQ-Custom-Song-List-Game.user.js
...
// ANN Song ID bulk paste handler
document.addEventListener("paste", function (e) {
    const target = e.target;

    if (!target || target.id !== "cslgAnisongdbQueryInput") {
        return;
    }
    ...
    console.log("RAW PASTE:", JSON.stringify(text));

    // Only modify bulk pastes when using ANN Song ID mode
    const mode = $("#cslgAnisongdbModeSelect").val().toLowerCase();
    ...
    console.log("ANN SONG IDS:", ids);

    if (!ids.length) {
        console.warn("⚠️ No valid ANN Song IDs found.");
        return;
    }

    // The existing CSL code expects whitespace-separated IDs
...
async function getAnisongdbData(mode, query, filters) {
    ...
    if (songQueries.length === 1) {
        ...
        $("#cslgSongListCount").text(
            `Searching ${songQueries.length} songs...`
        );

// Search songs one at a time
for (let i = 0; i < songQueries.length; i++) {
    ...
    try {
        const response = await fetch(apiBase + "search_request", {
            method: "POST",
            headers: {
                "Accept": "application/json",
                "Content-Type": "application/json",
                "X-Client-Id": "amqCustomSongListGame"
            },
        ...
        if (!response.ok) {
            console.warn(
                `AnisongDB failed for "${songQuery}": ${response.status}`
            );
        } else {
        ...
        console.warn(
            `AnisongDB request failed for "${songQuery}":`,
            error
        );
    }
    ...
    $("#cslgSongListCount").text(
        `Searching ${i + 1} / ${songQueries.length} songs...`
    );
    ...
    await new Promise(resolve => setTimeout(resolve, 300));
        ...
        for (const song of results) {
            const key =
                song.annSongId ??
                song.songInfo?.annSongId ??
                `${song.songName ?? song.name ?? ""}||${song.songArtist ?? song.artist ?? ""}`;
        ...
        // Process using the existing CSLG code
        handleData(uniqueResults);
        setSongListTableSort();
        createSongListTable(true);
        createAnswerTable();
